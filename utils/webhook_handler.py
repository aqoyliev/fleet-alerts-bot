...
def _get_event_type(event: dict) -> str:
    ...
    # is re-derived — _format_event does that internally.
...
async def get_motive_api_key(_company_slug: str) -> str | None:
    """The Motive API key for this deployment.

    This bot serves one company, so the key is an env var rather than the per-company
    companies.motive_api_key column the multi-company bot reads. Kept async and
    slug-taking so the confirmation code below is identical between the two."""
    return config.MOTIVE_API_KEY or None


async def _motive_crash_is_real(event: dict, company_slug: str,
                                elapsed_before: float = 0.0) -> bool | None:
    ...
    configured, or the lookup failed). None must fail open: no evidence is not
    ...
    if not api_key:
        logger.warning(f"[motive] No API key configured (MOTIVE_API_KEY) — "
...
async def _note_crash(coro, event_id) -> None:
    ...
    everything, so an unguarded failure here — an unmigrated database being the
    obvious one, since schemas.sql is applied by hand — would abandon the whole
    event and silently drop a crash alert. Losing the row is the acceptable
    failure; losing the alert is not."""
    try:
        await coro
    except Exception as e:
...
async def _handle_event(bot: Bot, event: dict, company_slug: str = "",
    ...
    try:
        ...
        # before opening the crash channel. Samsara crashes are resolved by their own
        # poll — and have already sent their first card from _on_first — so they are
        # excluded here.
        if event_type == "crash" and event.get("_source") != "samsara":
            ...
            if crash_resume_elapsed is None:
                await _note_crash(record_pending(crash_key, company_slug, event), crash_key)
            listed = await _motive_crash_is_real(event, company_slug,
                                                 crash_resume_elapsed or 0.0)
...
async def resume_pending_crash_confirmations(bot: Bot) -> None:
    ...
    for row in pending:
        ...
        try:
            ...
            logger.info(f"[motive] Resuming crash confirmation {event_id} "
                        f"for company='{row['company_slug']}' ({int(age)}s held)")
            asyncio.create_task(
                _handle_event(bot, payload, row["company_slug"], crash_resume_elapsed=age)
            )
        except Exception as e:
